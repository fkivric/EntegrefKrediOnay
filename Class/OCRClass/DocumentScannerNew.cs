...
namespace EntegrefKrediOnay.Class.OCRClass
{
    ...
    public class DocumentScannerNew : DocumentCreationForm
    {
        ...
        public DocumentScannerNew(FtpClient ftpClient, string curID, bool forProduct = false)
        {
        ...
        public void DocumentCreateCustomer()
        {
            try
            {
                if (!string.IsNullOrEmpty(curID))
                {
                    ...
                    _ftpClient.Connect();
                    _ftpClient.BeginCreateDirectory(curID + "/Araştırmalar/", force: true, callback: null, state : null );
                    _ftpClient.Connect();
                    _ftpClient.BeginCreateDirectory(curID + "/Faturalar/", force: true, callback: null, state: null);
                    _ftpClient.Connect();
                    _ftpClient.BeginCreateDirectory(curID + "/Sözleşmeler/", force: true, callback: null, state: null);
                    _ftpClient.Connect();
                    _ftpClient.BeginCreateDirectory(curID + "/Teslimatlar/", force: true, callback: null, state: null);
                    _ftpClient.Connect();
                    _ftpClient.BeginCreateDirectory(curID + "/İrsaliye/", force: true, callback: null, state: null);
                    _ftpClient.Connect();
                    _ftpClient.BeginCreateDirectory(curID + "/Sevk Belgeleri/", force: true, callback: null, state: null);
                    _ftpClient.Connect();
                    _ftpClient.BeginCreateDirectory(curID + "/Arıza Belgeleri/", force: true, callback: null, state: null);
                    _ftpClient.Connect();
                    _ftpClient.BeginCreateDirectory(curID + "/KVKK Belgeleri/", force: true, callback: null, state: null);
                    _ftpClient.Connect();
                    _ftpClient.BeginCreateDirectory(curID + "/Müşteriye Özel Belgeler/", force: true, callback: null, state: null);
                    _ftpClient.Connect();
                    _ftpClient.BeginCreateDirectory(curID + "/Yüklenen Dosyalar/", force: true, callback: null, state: null);
                    _ftpClient.Connect();
                    _ftpClient.BeginCreateDirectory(curID + "/Teklifler/", force: true, callback: null, state: null);
                    _ftpClient.Connect();
                }
            }
        ...
        public void DocumentCreateCustomer(string curID, string folderName)
        {
            try
            {
                ...
                _ftpClient.SocketPollInterval = 0;
                _ftpClient.Connect();
                _ftpClient.BeginCreateDirectory(curID + "/" + folderName + "/", force: true, callback: null, state: null);
            }
            catch (FtpException fe)
            {
                MessageBox.Show("Dosya yüklenirken bir hata ile karşılaşıldı, yükleme iptal edildi! " + fe.Message);
                ...
                if (results != null && results.Count > 0)
                {
                    foreach (Result item in results)
                    {
                        try
                        {
                            ...
                            if (!images.Contains(bitmap))
                            {
                                if (Properties.Settings.Default.Company == "YONAVM" || Properties.Settings.Default.Company == "KAMALAR")
                                {
                                    if (splited.Length == 1)
                                    {
        ...
        public void ScanPdf(string path)
        {
            ...
            try
            {
                ...
                if (forProduct)
                {
                    _ftpClient.UploadFile(path, "Ürünler/" + curID + "/Yüklenen Dosyalar/" + path.Substring(index, path.Length - index), FtpExists.Overwrite, createRemoteDir: true);
                }
                else
                {
                    _ftpClient.UploadFile(path, curID + "/Yüklenen Dosyalar/" + path.Substring(index, path.Length - index), FtpExists.Overwrite, createRemoteDir: true);
                }
                _ftpClient.Disconnect();
            }
            catch (FtpException fe)
        ...
        private void ScanningComplete()
        {
            ...
            {
                PdfDocument pdf = new PdfDocument();
                foreach (eBarcodeInformation document in lBarcodeInformation)
                {
                    PdfPage newPage = new PdfPage();
                    pdf.Pages.Add(newPage);
                    newPage.Size = PageSize.A4;
                ...
                pdf.Close();
                string curId = null;
                if (curID == null && curId == null)
                {
                    return;
                }
                if (curID != null && curID != "")
                {
                    string date = DateTime.Now.Day + "-" + DateTime.Now.Month + "-" + DateTime.Now.Year + " " + DateTime.Now.TimeOfDay.ToString();
                    if (!forProduct)
                    {
                        _ftpClient.Upload(fileStream, curID + "/Müşteriye Özel Belgeler/Özel Belge " + date + ".pdf", FtpExists.Overwrite, createRemoteDir: true);
                    }
                    ...
                    {
                        _ftpClient.Upload(fileStream, "Ürünler/" + curID + "/Taranan Dosyalar/Ürün Belgesi " + DateTime.Now.ToString() + ".pdf", FtpExists.Overwrite, createRemoteDir: true);
                    }
                }
                else if (curId != null && curId != "")
                {
                    string date2 = DateTime.Now.Day + "-" + DateTime.Now.Month + "-" + DateTime.Now.Year + " " + DateTime.Now.TimeOfDay.ToString();
                    _ftpClient.Upload(fileStream, curId + "/Müşteriye Özel Belgeler/Özel Belge " + date2 + ".pdf", FtpExists.Overwrite, createRemoteDir: true);
                }
            }
            catch (FtpException fe)
            {
            ...
            {
                successfullyScannedDocuments.AddRange(lBarcodeInformation);
                _ftpClient.Disconnect();
                isFinished = true;
                SplashScreenManager.CloseForm(throwExceptionIfAlreadyClosed: false);
            }
